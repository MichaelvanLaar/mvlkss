...
- **PHPUnit** for PHP unit and integration tests (`tests/Unit/`, `tests/Integration/`)
- **Vitest** with jsdom for JavaScript unit tests (`tests/js/`)
- **PHPStan** (level 6, with baseline) for PHP static analysis
- **ESLint**, **Stylelint**, **yamllint** for linting
- **CI** runs all of the above on GitHub Actions across PHP 8.2, 8.3, 8.4
- Run everything locally with `npm test`; see `TESTING.md` for details
...
2. **PHP Version**: Requires PHP >= 8.2.0
