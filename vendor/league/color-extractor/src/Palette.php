...
class Palette implements \Countable, \IteratorAggregate {
    ...
    public static function fromFilename($filename, $backgroundColor = null) {
        if (!is_readable($filename)) {
            throw new \InvalidArgumentException(
                "Filename must be a valid path and should be readable",
            );
        }
    ...
    public static function fromGD($image, ?int $backgroundColor = null) {
        ...
        if (
            ...
            (!is_numeric($backgroundColor) ||
                $backgroundColor < 0 ||
                $backgroundColor > 16777215)
        ) {
            throw new \InvalidArgumentException(
                sprintf(
                    '"%s" does not represent a valid color',
                    $backgroundColor,
                ),
            );
        }
        ...
        for ($x = 0; $x < $imageWidth; ++$x) {
            for ($y = 0; $y < $imageHeight; ++$y) {
                ...
                if ($areColorsIndexed) {
                    ...
                    $color =
                        $colorComponents["alpha"] * 16777216 +
                ...
                if ($alpha = $color >> 24) {
                    ...
                    $alpha /= 127;
                    $color =
                        (int) ((($color >> 16) & 0xff) * (1 - $alpha) +
                            $backgroundColorRed * $alpha) *
                            65536 +
                        (int) ((($color >> 8) & 0xff) * (1 - $alpha) +
                            $backgroundColorGreen * $alpha) *
                            256 +
                        (int) (($color & 0xff) * (1 - $alpha) +
                            $backgroundColorBlue * $alpha);
                }
