...
class PaletteTest extends TestCase {
    ...
    public function testTransparencyHandling() {
        ...
        $whiteBackgroundPalette = Palette::fromFilename(
            $this->transparentPngPath,
            Color::fromHexToInt("#FFFFFF"),
        );
        $this->assertEquals(iterator_to_array($whiteBackgroundPalette), [
            Color::fromHexToInt("#FF8080") => 1,
        ]);

        $blackBackgroundPalette = Palette::fromFilename(
            $this->transparentPngPath,
            Color::fromHexToInt("#000000"),
        );
        $this->assertEquals(iterator_to_array($blackBackgroundPalette), [
            Color::fromHexToInt("#7E0000") => 1,
        ]);
    }
}
