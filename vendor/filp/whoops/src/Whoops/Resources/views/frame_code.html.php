...
<div class="frame-code-container <?php echo !$has_frames ? "empty" : ""; ?>">
  <?php foreach ($frames as $i => $frame): ?>
    <?php $line = $frame->getLine(); ?>
      <div class="frame-code <?php echo $i == 0
        ...
        <div class="frame-file">
          ...
          <?php if (
              $filePath &&
              ($editorHref = $handler->getEditorHref($filePath, (int) $line))
          ): ?>
            <a href="<?php echo $editorHref; ?>" class="editor-link"<?php echo $handler->getEditorAjax(
    $filePath,
    (int) $line,
)
    ? " data-ajax"
    : ""; ?>>
              Open:
              <strong><?php echo $tpl->breakOnDelimiter(
                  "/",
                  $tpl->escape($filePath ?: "<#unknown>"),
              ); ?></strong>
            </a>
          <?php else: ?>
            <strong><?php echo $tpl->breakOnDelimiter(
        ...
        <?php // Do nothing if there's no line to work off
      ...
      if ($line !== null):
            ...
            ><code class="language-php"><?php echo $tpl->escape(
                $code,
            ); ?></code></pre>
        ...
        <?php // Append comments for this frame
      ...
      $comments = $frame->getComments(); ?>
        <div class="frame-comments <?php echo empty($comments)
          ...
          <?php foreach ($comments as $commentNo => $comment): ?>
            <?php extract($comment); ?>
            <div class="frame-comment" id="comment-<?php echo $i .
                "-" .
                $commentNo; ?>">
              <span class="frame-comment-context"><?php echo $tpl->escape(
                  $context,
              ); ?></span>
