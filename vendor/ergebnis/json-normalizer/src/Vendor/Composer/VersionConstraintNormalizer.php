...
final class VersionConstraintNormalizer implements Normalizer {
    ...
    public function normalize(Json $json): Json {
        ...
        foreach ($objectPropertiesThatShouldBeNormalized as $name => $value) {
            ...
            $decoded->{$name} = \array_map(function (
                string $versionConstraint,
            ): string {
                $versionConstraint = self::trim($versionConstraint);
                $versionConstraint = self::removeExtraSpaces(
                    $versionConstraint,
                );
    ...
    private function normalizeVersionConstraint(
        string $versionConstraint,
    ): string {
        $versionConstraint = self::normalizeVersionConstraintSeparators(
            $versionConstraint,
        );
        $versionConstraint = self::removeLeadingVersionPrefix(
            $versionConstraint,
        );
        $versionConstraint = self::assertDevPrefixSuffixPosition(
            $versionConstraint,
        );
        $versionConstraint = self::replaceWildcardXWithAsterisk(
            $versionConstraint,
        );
        ...
        $versionConstraint = self::removeDuplicateVersionConstraints(
            $versionConstraint,
        );
        $versionConstraint = self::removeUselessInlineAliases(
            $versionConstraint,
        );
    ...
    private static function removeExtraSpaces(
        string $versionConstraint,
    ): string {
        return \preg_replace("/ +/", " ", $versionConstraint);
    }

    private static function normalizeVersionConstraintSeparators(
        string $versionConstraint,
    ): string {
        $orConstraints = self::splitIntoOrConstraints($versionConstraint);

        return self::joinOrConstraints(
    ...
    private static function replaceWildcardXWithAsterisk(
        string $versionConstraint,
    ): string {
        // '1.x.x' -> '1.*'
    ...
    private static function replaceWildcardWithTilde(
        string $versionConstraint,
    ): string {
        return self::applyRegularExpressionReplacementToVersionsInTurn(
            $versionConstraint,
    ...
    private static function replaceTildeWithCaret(
        string $versionConstraint,
    ): string {
        return self::applyRegularExpressionReplacementToVersionsInTurn(
            $versionConstraint,
    ...
    private static function removeDuplicateVersionConstraints(
        string $versionConstraint,
    ): string {
        $orConstraints = self::splitIntoOrConstraints($versionConstraint);
        ...
        return self::joinOrConstraints(
            ...\array_unique(
                \array_map(static function (string $orConstraint): string {
                    $andConstraints = self::splitIntoAndConstraints(
                        $orConstraint,
                    );

                    return self::joinAndConstraints(
                        ...\array_unique($andConstraints),
                    );
                }, $orConstraints),
            ),
        );
    ...
    private static function removeLeadingVersionPrefix(
        string $versionConstraint,
    ): string {
        return self::applyRegularExpressionReplacementToVersionsInTurn(
            $versionConstraint,
    ...
    private static function assertDevPrefixSuffixPosition(
        string $versionConstraint,
    ): string {
    ...
    private static function removeOverlappingVersionConstraints(
        string $versionConstraint,
    ): string {
        $orConstraints = self::splitIntoOrConstraints($versionConstraint);
        ...
        return self::joinOrConstraints(
            ...\array_filter($orConstraints, static function (
                ?string $orConstraint,
            ): bool {
    ...
    private static function removeUselessInlineAliases(
        string $normalized,
    ): string {
    ...
    private static function sortVersionConstraints(
        string $versionConstraint,
    ): string {
        ...
        $orConstraints = \array_map(static function (string $orConstraint) use (
            $sort,
        ): string {
            $andConstraints = self::splitIntoAndConstraints($orConstraint);
     ...
     * @return list<string>
     */
    private static function splitIntoOrConstraints(
        string $versionConstraint,
    ): array {
        return \preg_split("{\s*\|\|?\s*}", $versionConstraint);
    }

    private static function joinOrConstraints(
        string ...$orConstraints,
    ): string {
     ...
     * @return list<string>
     */
    private static function splitIntoAndConstraints(
        string $orConstraint,
    ): array {
        return \preg_split(
            '{(?<!^|as|[=>< ,]) *(?<!-)[, ](?!-) *(?!,|as|$)}',
    ...
    private static function joinAndConstraints(
     ...
     */
    private static function applyRegularExpressionReplacementToVersionsInTurn(
        string $versionConstraint,
        string $find,
        string $replace,
    ): string {
        ...
        foreach ($split as &$part) {
            $part = \preg_replace($find, $replace, $part);
        }
