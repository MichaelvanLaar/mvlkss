...
final class ReferenceToken {
    ...
    public static function fromJsonString(string $value): self {
        ...
        return new self(\str_replace(["~1", "~0"], ["/", "~"], $value));
    }
    ...
    public static function fromUriFragmentIdentifierString(
        ...
        if (
            1 !==
            \preg_match(
                Pattern::URI_FRAGMENT_IDENTIFIER_REFERENCE_TOKEN,
                $value,
            )
        ) {
            throw Exception\InvalidReferenceToken::fromJsonString($value);
        ...
        return new self(
            \str_replace(["~1", "~0"], ["/", "~"], \rawurldecode($value)),
        );
    }

    public function toJsonString(): string {
        return \str_replace(["~", "/"], ["~0", "~1"], $this->value);
    }

    public function toUriFragmentIdentifierString(): string {
        return \rawurlencode(
            \str_replace(["~", "/"], ["~0", "~1"], $this->value),
        );
    }
