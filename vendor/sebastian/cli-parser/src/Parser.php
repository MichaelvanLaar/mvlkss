...
final class Parser {
    ...
    public function parse(
        ...
        while (false !== ($arg = current($argv))) {
            ...
            if ($arg === "--") {
                $nonOptions = array_merge(
                    $nonOptions,
                    array_slice($argv, $i + 1),
                );
            ...
            if (
                $arg[0] !== "-" ||
                (strlen($arg) > 1 && $arg[1] === "-" && $longOptions === null)
            ) {
                $nonOptions[] = $arg;
    ...
    private function parseShortOption(
        ...
        for ($i = 0; $i < $argumentLength; $i++) {
            ...
            if (
                $argument[$i] === ":" ||
                ($spec = strstr($shortOptions, $option)) === false
            ) {
    ...
    private function parseLongOption(
        ...
        foreach ($longOptions as $i => $longOption) {
            ...
            if (str_ends_with($longOption, "=")) {
                if (
                    !str_ends_with($longOption, "==") &&
                    !strlen((string) $optionArgument)
                ) {
