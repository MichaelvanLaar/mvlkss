 ...
 */
final class StrictUnifiedDiffOutputBuilder implements
    ...
    public function __construct(array $options = []) {
        ...
        $this->header = sprintf(
            "--- %s%s\n+++ %s%s\n",
            $options["fromFile"],
            null === $options["fromFileDate"]
                ? ""
                : "\t" . $options["fromFileDate"],
            $options["toFile"],
            null === $options["toFileDate"]
                ? ""
                : "\t" . $options["toFileDate"],
        );
    ...
    private function writeDiffHunks($output, array $diff): void {
        ...
        foreach ($diff as $i => $entry) {
            if (0 === $entry[1]) {
                ...
                if ($sameCount === $cutOff) {
                    $contextStartOffset =
                        $hunkCapture - $this->contextLines < 0
                            ? $hunkCapture
                            : $this->contextLines;
