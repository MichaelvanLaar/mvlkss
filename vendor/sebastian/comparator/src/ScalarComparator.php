...
class ScalarComparator extends Comparator {
    ...
    public function assertEquals(
        ...
        // otherwise 0 == 'Foobar'
        if (
            (is_string($expected) && !is_bool($actual)) ||
            (is_string($actual) && !is_bool($expected))
        ) {
        ...
        if (
            $expectedToCompare !== $actualToCompare &&
            is_string($expected) &&
            is_string($actual)
        ) {
            [$cutExpected, $cutActual] = self::removeOverlongCommonPrefix(
                $expected,
                $actual,
            );
            [$cutExpected, $cutActual] = self::removeOverlongCommonSuffix(
                $cutExpected,
                $cutActual,
            );
     ...
     */
    private static function removeOverlongCommonPrefix(
        string $string1,
        string $string2,
    ): array {
        ...
        if (strlen($commonPrefix) > self::OVERLONG_THRESHOLD) {
            $string1 =
                ...
                substr(
                    $string1,
                    strlen($commonPrefix) - self::KEEP_CONTEXT_CHARS,
                );
            $string2 =
                "..." .
                substr(
    ...
    private static function findCommonPrefix(
        string $string1,
        string $string2,
    ): string {
     ...
     * @return array{string, string}
     */
    private static function removeOverlongCommonSuffix(
        string $string1,
        string $string2,
    ): array {
        ...
        if (strlen($commonSuffix) > self::OVERLONG_THRESHOLD) {
            $string1 =
                substr(
                    $string1,
                    0,
                    -(strlen($commonSuffix) - self::KEEP_CONTEXT_CHARS),
                ) . "...";
            $string2 =
                substr(
                    $string2,
    ...
    private static function findCommonSuffix(
        string $string1,
        string $string2,
    ): string {
        if ($string1 === "" || $string2 === "") {
