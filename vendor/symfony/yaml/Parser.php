...
class Parser {
    ...
    private function doParse(string $value, int $flags): mixed {
        ...
        do {
            ...
            if (
                ...
                if (
                    isset($values["value"]) &&
                    str_starts_with(ltrim($values["value"], " "), "-")
                ) {
                    // Inline first child
                ...
                (!str_contains($values["key"], " #") ||
                    \in_array($values["key"][0], ['"', "'"], true))
            ) {
                        ...
                        if ($allowOverwrite || !isset($data[$key])) {
                            if (
                                ...
                                trigger_deprecation(
                                    "symfony/yaml",
                                    "7.2",
                                    'Duplicate key "%s" detected on line %d whilst parsing YAML. Silent handling of duplicate mapping keys in YAML is deprecated and will throw a ParseException in 8.0.',
                                    $key,
                                    $this->getRealCurrentLineNb() + 1,
                                );
                            }
    ...
    private function lexUnquotedString(int &$cursor): string {
        ...
        while ($cursor < \strlen($this->currentLine)) {
            ...
            if (
                \in_array($this->currentLine[$cursor], [" ", "\t"], true) &&
                "#" === ($this->currentLine[$cursor + 1] ?? "")
            ) {
                break;
            }
    ...
    private function lexInlineMapping(
        int &$cursor = 0,
        bool $consumeUntilEol = true,
    ): string {
        return $this->lexInlineStructure($cursor, "}", $consumeUntilEol);
    }

    private function lexInlineSequence(
        int &$cursor = 0,
        bool $consumeUntilEol = true,
    ): string {
        return $this->lexInlineStructure($cursor, "]", $consumeUntilEol);
                ...
                switch ($this->currentLine[$cursor]) {
                    ...
                    case $closingTag:
                        ...
                        if (
                            ...
                            isset($this->currentLine[$cursor]) &&
                            ($whitespaces =
                                strspn($this->currentLine, " ", $cursor) +
                                $cursor) < \strlen($this->currentLine) &&
                            "#" !== $this->currentLine[$whitespaces]
                        ) {
                            throw new ParseException(
                                \sprintf(
                                    'Unexpected token "%s".',
                                    trim(substr($this->currentLine, $cursor)),
                                ),
                            );
                        }
