...
class Dumper {
    ...
    public function dump(
        ...
        if (
            $flags & Yaml::DUMP_NULL_AS_EMPTY &&
            $flags & Yaml::DUMP_NULL_AS_TILDE
        ) {
            throw new \InvalidArgumentException(
                "The Yaml::DUMP_NULL_AS_EMPTY and Yaml::DUMP_NULL_AS_TILDE flags cannot be used together.",
            );
        }
    ...
    private function doDump(
        ...
        if (
            ...
            $output .=
                $prefix . Inline::dump($input, $flags, 0 === $nestingLevel);
        } elseif ($input instanceof TaggedValue) {
            $output .= $this->dumpTaggedValue(
                $input,
                $inline,
                $indent,
                $flags,
            ...
            $compactNestedMapping =
                Yaml::DUMP_COMPACT_NESTED_MAPPING & $flags && !$dumpAsMap;
            ...
            foreach ($input as $key => $value) {
                ...
                if (
                    \is_int($key) &&
                    Yaml::DUMP_NUMERIC_KEY_AS_STRING & $flags
                ) {
                    $key = (string) $key;
                }
                ...
                if (
                    ...
                    $blockIndentationIndicator = $this->getBlockIndentationIndicator(
                        $value,
                    );
    ...
    private function dumpTaggedValue(
        TaggedValue $value,
        int $inline,
        int $indent,
        int $flags,
