...
class Inline {
    ...
    public static function dump(
        ...
        switch (true) {
            ...
            case Escaper::requiresSingleQuoting($value):
                ...
                return \strlen($doubleQuoted) < \strlen($singleQuoted)
                    ? $doubleQuoted
                    : $singleQuoted;
     ...
     * @param int                          $flags A bit field of Yaml::DUMP_* constants to customize the dumped YAML string
     */
    private static function dumpHashArray(
    ...
    private static function parseSequence(
        ...
        while ($i < $len) {
            ...
            if ("," === $sequence[$i] || " " === $sequence[$i]) {
                if (
                    "," === $sequence[$i] &&
                    (null === $lastToken || "separator" === $lastToken)
                ) {
                    $output[] = null;
                } elseif ("," === $sequence[$i]) {
                    $lastToken = "separator";
                ...
                switch (true) {
                    ...
                    case str_starts_with($scalar, "!php/enum"):
                        if (self::$constantSupport) {
                            if (!isset($scalar[11])) {
                                throw new ParseException(
                                    'Missing value for tag "!php/enum".',
                                    self::$parsedLineNumber + 1,
                                    $scalar,
                                    self::$parsedFilename,
                                );
                            ...
                            $enum = $useName
                                ? strstr($enumName, "::", true)
                                : $enumName;

                            if (!enum_exists($enum)) {
                                throw new ParseException(
                                    \sprintf(
                                        'The enum "%s" is not defined.',
                                        $enum,
                                    ),
                                    self::$parsedLineNumber + 1,
                                    $scalar,
                            ...
                            if (
                                $useValue = str_ends_with($enumName, "->value")
                            ) {
                            ...
                            if (!\defined($enumName)) {
                                throw new ParseException(
                                    \sprintf(
                                        'The string "%s" is not the name of a valid enum.',
                                        $enumName,
                                    ),
                                    self::$parsedLineNumber + 1,
                                    $scalar,
                            ...
                            if (!($value instanceof \BackedEnum)) {
                                throw new ParseException(
                                    \sprintf(
                                        'The enum "%s" defines no value next to its name.',
                                        $enumName,
                                    ),
                                    self::$parsedLineNumber + 1,
                                    $scalar,
                        ...
                        if (self::$exceptionOnInvalidType) {
                            throw new ParseException(
                                \sprintf(
                                    'The string "%s" could not be parsed as an enum. Did you forget to pass the "Yaml::PARSE_CONSTANT" flag to the parser?',
                                    $scalar,
                                ),
                                self::$parsedLineNumber + 1,
                                $scalar,
                ...
                switch (true) {
                    ...
                    case Parser::preg_match(self::getTimestampRegex(), $scalar):
                        try {
                            ...
                            throw new ParseException(
                                \sprintf(
                                    'The date "%s" could not be parsed as it is an invalid date.',
                                    $scalar,
                                ),
                                self::$parsedLineNumber + 1,
                                $scalar,
    ...
    private static function parseTag(
        ...
        if (
            ...
            ("!" === $tag[0] ||
                \in_array(
                    $tag,
                    ["str", "php/const", "php/enum", "php/object"],
                    true,
                ))
        ) {
