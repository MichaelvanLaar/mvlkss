...
class Parser implements ParserInterface {
    ...
    private function parseSelectorList(
        TokenStream $stream,
    ...
    private function parserSelectorNode(
        TokenStream $stream,
        bool $isArgument = false,
    ): Node\SelectorNode {
        [$result, $pseudoElement] = $this->parseSimpleSelector(
            $stream,
            false,
            $isArgument,
        );
        ...
        while (true) {
            ...
            [$nextSelector, $pseudoElement] = $this->parseSimpleSelector(
                $stream,
                false,
                $isArgument,
            );
        ...
        while (true) {
            ...
            if ($peek->isHash()) {
                ...
                if (!$stream->getPeek()->isDelimiter(["("])) {
                    ...
                    if ("Pseudo[Element[*]:scope]" === $result->__toString()) {
                        ...
                        if (
                            ...
                            throw SyntaxErrorException::notAtTheStartOfASelector(
                                "scope",
                            );
                        }
                    }
                ...
                if ("not" === strtolower($identifier)) {
                    ...
                    $result = new Node\SpecificityAdjustmentNode(
                        $result,
                        $selectors,
                    );
                } else {
