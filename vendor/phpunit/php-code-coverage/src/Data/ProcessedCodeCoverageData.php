...
final class ProcessedCodeCoverageData {
        ...
        foreach ($rawData->lineCoverage() as $file => $lines) {
            if (!isset($this->lineCoverage[$file])) {
                ...
                foreach ($lines as $k => $v) {
                    $this->lineCoverage[$file][$k] =
                        $v === Driver::LINE_NOT_EXECUTABLE ? null : [];
                }
            }
        }
        ...
        foreach ($rawData->functionCoverage() as $file => $functions) {
            foreach ($functions as $functionName => $functionData) {
                if (isset($this->functionCoverage[$file][$functionName])) {
                    $this->initPreviouslySeenFunction(
                        $file,
                        $functionName,
                        $functionData,
                    );
                } else {
                    $this->initPreviouslyUnseenFunction(
                        $file,
                        $functionName,
                        $functionData,
                    );
    ...
    public function markCodeAsExecutedByTestCase(
        string $testCaseId,
        RawCodeCoverageData $executedCode,
    ): void {
        ...
        foreach ($executedCode->functionCoverage() as $file => $functions) {
            foreach ($functions as $functionName => $functionData) {
                foreach (
                    $functionData["branches"]
                    as $branchId => $branchData
                ) {
                    if ($branchData["hit"] === Driver::BRANCH_HIT) {
                        $this->functionCoverage[$file][$functionName][
                            "branches"
                        ][$branchId]["hit"][] = $testCaseId;
                    }
                }

                foreach ($functionData["paths"] as $pathId => $pathData) {
                    if ($pathData["hit"] === Driver::BRANCH_HIT) {
                        $this->functionCoverage[$file][$functionName]["paths"][
                            $pathId
                        ]["hit"][] = $testCaseId;
                    }
                }
    ...
    public function merge(self $newData): void {
        foreach ($newData->lineCoverage as $file => $lines) {
            ...
            foreach ($compareLineNumbers as $line) {
                $thatPriority = $this->priorityForLine(
                    $newData->lineCoverage[$file],
                    $line,
                );
                $thisPriority = $this->priorityForLine(
                    $this->lineCoverage[$file],
                    $line,
                );

                if ($thatPriority > $thisPriority) {
                    $this->lineCoverage[$file][$line] =
                        $newData->lineCoverage[$file][$line];
                } elseif (
                    $thatPriority === $thisPriority &&
     ...
     */
    private function initPreviouslyUnseenFunction(
        string $file,
        string $functionName,
        array $functionData,
    ): void {
        ...
        foreach (array_keys($functionData["paths"]) as $pathId) {
            $this->functionCoverage[$file][$functionName]["paths"][$pathId][
                "hit"
            ] = [];
        }
    }
     ...
     * @param XdebugFunctionCoverageType $functionData
     */
    private function initPreviouslySeenFunction(
        string $file,
        string $functionName,
        array $functionData,
    ): void {
