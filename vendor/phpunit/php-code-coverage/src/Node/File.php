...
final class File extends AbstractNode {
    ...
    public function __construct(
        string $name,
        AbstractNode $parent,
        array $lineCoverageData,
        array $functionCoverageData,
        array $testData,
        array $classes,
        array $traits,
        array $functions,
    ...
    private function calculateStatistics(
        ...
        foreach ($this->traits as &$trait) {
            foreach ($trait["methods"] as &$method) {
                $methodLineCoverage = $method["executableLines"]
                    ? ($method["executedLines"] / $method["executableLines"]) *
                        100
                    : 100;
                $methodBranchCoverage = $method["executableBranches"]
                    ? ($method["executedBranches"] /
                            $method["executableBranches"]) *
                        100
                    : 0;
                $methodPathCoverage = $method["executablePaths"]
                    ? ($method["executedPaths"] / $method["executablePaths"]) *
                        100
                    : 0;

                $method["coverage"] =
                    $methodBranchCoverage ?: $methodLineCoverage;
                $method["crap"] = (new CrapIndex(
                    $method["ccn"],
            ...
            $traitLineCoverage = $trait["executableLines"]
                ? ($trait["executedLines"] / $trait["executableLines"]) * 100
                : 100;
            $traitBranchCoverage = $trait["executableBranches"]
                ? ($trait["executedBranches"] / $trait["executableBranches"]) *
                    100
                : 0;
            $traitPathCoverage = $trait["executablePaths"]
                ? ($trait["executedPaths"] / $trait["executablePaths"]) * 100
                : 0;

            $trait["coverage"] = $traitBranchCoverage ?: $traitLineCoverage;
            $trait["crap"] = (new CrapIndex(
                $trait["ccn"],
        ...
        foreach ($this->classes as &$class) {
            ...
            $classLineCoverage = $class["executableLines"]
                ? ($class["executedLines"] / $class["executableLines"]) * 100
                : 100;
            $classBranchCoverage = $class["executableBranches"]
                ? ($class["executedBranches"] / $class["executableBranches"]) *
                    100
                : 0;
            $classPathCoverage = $class["executablePaths"]
                ? ($class["executedPaths"] / $class["executablePaths"]) * 100
                : 0;

            $class["coverage"] = $classBranchCoverage ?: $classLineCoverage;
            $class["crap"] = (new CrapIndex(
                $class["ccn"],
        ...
        foreach ($this->functions as &$function) {
            $functionLineCoverage = $function["executableLines"]
                ? ($function["executedLines"] / $function["executableLines"]) *
                    100
                : 100;
            $functionBranchCoverage = $function["executableBranches"]
                ? ($function["executedBranches"] /
                        $function["executableBranches"]) *
                    100
                : 0;
            $functionPathCoverage = $function["executablePaths"]
                ? ($function["executedPaths"] / $function["executablePaths"]) *
                    100
                : 0;

            $function["coverage"] =
                $functionBranchCoverage ?: $functionLineCoverage;
            $function["crap"] = (new CrapIndex(
                $function["ccn"],
    ...
    private function processClasses(array $classes): void {
        ...
        foreach ($classes as $className => $class) {
            ...
            foreach ($class["methods"] as $methodName => $method) {
                ...
                $this->classes[$className]["executableBranches"] +=
                    $methodData["executableBranches"];
                $this->classes[$className]["executedBranches"] +=
                    $methodData["executedBranches"];
                $this->classes[$className]["executablePaths"] +=
                    $methodData["executablePaths"];
                $this->classes[$className]["executedPaths"] +=
                    $methodData["executedPaths"];

                $this->numExecutableBranches +=
                    $methodData["executableBranches"];
                ...
                foreach (
                    range($method["startLine"], $method["endLine"])
                    as $lineNumber
                ) {
                    $this->codeUnitsByLine[$lineNumber] = [
                        &$this->classes[$className],
    ...
    private function processTraits(array $traits): void {
        ...
        foreach ($traits as $traitName => $trait) {
            ...
            foreach ($trait["methods"] as $methodName => $method) {
                ...
                $this->traits[$traitName]["executableBranches"] +=
                    $methodData["executableBranches"];
                $this->traits[$traitName]["executedBranches"] +=
                    $methodData["executedBranches"];
                $this->traits[$traitName]["executablePaths"] +=
                    $methodData["executablePaths"];
                $this->traits[$traitName]["executedPaths"] +=
                    $methodData["executedPaths"];
    ...
    private function processFunctions(array $functions): void {
        ...
        foreach ($functions as $functionName => $function) {
            ...
            foreach (
                range($function["startLine"], $function["endLine"])
                as $lineNumber
            ) {
                $this->codeUnitsByLine[$lineNumber] = [
                    &$this->functions[$functionName],
            ...
            $this->numExecutableBranches +=
                $this->functions[$functionName]["executableBranches"];
            $this->numExecutedBranches +=
                $this->functions[$functionName]["executedBranches"];
            $this->numExecutablePaths +=
                $this->functions[$functionName]["executablePaths"];
            $this->numExecutedPaths +=
                $this->functions[$functionName]["executedPaths"];
        }
    }
