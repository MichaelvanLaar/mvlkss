    ...
    public function process(
        ...
        foreach ($report as $item) {
            ...
            foreach ($classes as $className => $class) {
                ...
                $xmlMetrics->setAttribute(
                    "conditionals",
                    (string) $class["executableBranches"],
                );
                $xmlMetrics->setAttribute(
                    "coveredconditionals",
                    (string) $class["executedBranches"],
                );
                $xmlMetrics->setAttribute(
                    "statements",
                    (string) $classStatements,
                );
                $xmlMetrics->setAttribute(
                    "coveredstatements",
                    (string) $coveredClassStatements,
                );
                $xmlMetrics->setAttribute(
                    "elements",
                    (string) ($classMethods +
            ...
            $xmlMetrics->setAttribute(
                "loc",
                (string) $linesOfCode["linesOfCode"],
            );
            $xmlMetrics->setAttribute(
                "ncloc",
                (string) $linesOfCode["nonCommentLinesOfCode"],
            );
            $xmlMetrics->setAttribute(
                "classes",
                (string) $item->numberOfClassesAndTraits(),
            );
            $xmlMetrics->setAttribute(
                "methods",
                (string) $item->numberOfMethods(),
            );
            $xmlMetrics->setAttribute(
                "coveredmethods",
                (string) $item->numberOfTestedMethods(),
            );
            $xmlMetrics->setAttribute(
                "conditionals",
                (string) $item->numberOfExecutableBranches(),
            );
            $xmlMetrics->setAttribute(
                "coveredconditionals",
                (string) $item->numberOfExecutedBranches(),
            );
            $xmlMetrics->setAttribute(
                "statements",
                (string) $item->numberOfExecutableLines(),
            );
            $xmlMetrics->setAttribute(
                "coveredstatements",
                (string) $item->numberOfExecutedLines(),
            );
            $xmlMetrics->setAttribute(
                "elements",
                (string) ($item->numberOfMethods() +
                    $item->numberOfExecutableLines() +
                    $item->numberOfExecutableBranches()),
            );
            $xmlMetrics->setAttribute(
                "coveredelements",
                (string) ($item->numberOfTestedMethods() +
                    $item->numberOfExecutedLines() +
                    $item->numberOfExecutedBranches()),
            );
        ...
        $xmlMetrics->setAttribute(
            "classes",
            (string) $report->numberOfClassesAndTraits(),
        );
        $xmlMetrics->setAttribute(
            "methods",
            (string) $report->numberOfMethods(),
        );
        $xmlMetrics->setAttribute(
            "coveredmethods",
            (string) $report->numberOfTestedMethods(),
        );
        $xmlMetrics->setAttribute(
            "conditionals",
            (string) $report->numberOfExecutableBranches(),
        );
        $xmlMetrics->setAttribute(
            "coveredconditionals",
            (string) $report->numberOfExecutedBranches(),
        );
        $xmlMetrics->setAttribute(
            "statements",
            (string) $report->numberOfExecutableLines(),
        );
        $xmlMetrics->setAttribute(
            "coveredstatements",
            (string) $report->numberOfExecutedLines(),
        );
        $xmlMetrics->setAttribute(
            "elements",
            (string) ($report->numberOfMethods() +
