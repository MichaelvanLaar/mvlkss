...
final class DocBlock {
    ...
    public static function ofClass(ReflectionClass $class): self {
        if ($class->isInternal()) {
            throw new AnnotationsAreNotSupportedForInternalClassesException(
                $class->getName(),
            );
        }
    ...
    public function requirements(): array {
        ...
        foreach ($lines as $line) {
            ...
            if (
                preg_match(
                    self::REGEX_REQUIRES_VERSION_CONSTRAINT,
                    $line,
                    $matches,
                )
            ) {
                ...
                try {
                    ...
                    $requires[$matches["name"] . "_constraint"] = [
                        "constraint" => $versionConstraintParser->parse(
                            trim($matches["constraint"]),
                        ),
                    ];

                    $recordedOffsets[
                        $matches["name"] . "_constraint"
                    ] = $offset;
     ...
     */
    private static function extractAnnotationsFromReflector(
