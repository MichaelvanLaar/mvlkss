...
final class ProgressPrinter {
    ...
    public function testTriggeredNotice(NoticeTriggered $event): void {
        ...
        if (
            !$this->source->ignoreSuppressionOfNotices() &&
            $event->wasSuppressed()
        ) {
            return;
        }
    ...
    public function testTriggeredPhpNotice(PhpNoticeTriggered $event): void {
        ...
        if (
            !$this->source->ignoreSuppressionOfPhpNotices() &&
            $event->wasSuppressed()
        ) {
            return;
        }
    ...
    public function testTriggeredDeprecation(
        ...
        if (
            $this->source->ignoreSelfDeprecations() &&
            $event->trigger()->isSelf()
        ) {
            return;
        }

        if (
            $this->source->ignoreDirectDeprecations() &&
            $event->trigger()->isDirect()
        ) {
            return;
        }

        if (
            $this->source->ignoreIndirectDeprecations() &&
            $event->trigger()->isIndirect()
        ) {
            return;
        }
        ...
        if (
            !$this->source->ignoreSuppressionOfDeprecations() &&
            $event->wasSuppressed()
        ) {
            return;
        }
    ...
    public function testTriggeredPhpDeprecation(
        ...
        if (
            !$this->source->ignoreSuppressionOfPhpDeprecations() &&
            $event->wasSuppressed()
        ) {
            return;
        }
    ...
    public function testTriggeredWarning(WarningTriggered $event): void {
        ...
        if (
            !$this->source->ignoreSuppressionOfWarnings() &&
            $event->wasSuppressed()
        ) {
            return;
        }
    ...
    public function testTriggeredPhpWarning(PhpWarningTriggered $event): void {
        ...
        if (
            !$this->source->ignoreSuppressionOfPhpWarnings() &&
            $event->wasSuppressed()
        ) {
            return;
        }
    ...
    public function testTriggeredError(ErrorTriggered $event): void {
        if (
            !$this->source->ignoreSuppressionOfErrors() &&
            $event->wasSuppressed()
        ) {
            return;
        }
    ...
    private function printProgressWithColor(
        string $color,
        string $progress,
    ): void {
        if ($this->colors) {
    ...
    private function printProgress(string $progress): void {
        ...
        if (
            ...
            $this->numberOfTestsRun === $this->numberOfTests
        ) {
            if ($this->numberOfTestsRun === $this->numberOfTests) {
                $this->printer->print(
                    str_repeat(" ", $this->maxColumn - $this->column),
                );
            }
            ...
            $this->printer->print(
                sprintf(
                    ...
                    $this->numberOfTests,
                    floor(
                        ($this->numberOfTestsRun / $this->numberOfTests) * 100,
                    ),
                ),
            );
