...
final readonly class ResultPrinter {
    ...
    public function print(TestResult $result, array $tests): void {
        ...
        foreach ($result->testErroredEvents() as $error) {
            if ($error instanceof BeforeFirstTestMethodErrored) {
                $beforeFirstTestMethodErrored[
                    $error->calledMethod()->className() .
                        "::" .
                        $error->calledMethod()->methodName()
                ] = $error;
            }
    ...
    private function printPrettifiedClassName(
    ...
    private function printTestResultHeader(TestDoxTestResult $test): void {
        ...
        $this->printer->print(
            $test->test()->testDox()->prettifiedMethodName($this->colors) .
                PHP_EOL,
        );
    }
    ...
    private function printTestResultBodyStart(TestDoxTestResult $test): void {
        $this->printer->print(
            $this->prefixLines($this->prefixFor("start", $test->status()), ""),
        );
    ...
    private function printTestResultBodyEnd(TestDoxTestResult $test): void {
        ...
        $this->printer->print(
            $this->prefixLines($this->prefixFor("last", $test->status()), ""),
        );
    ...
    private function printThrowable(
        ...
        if (!empty($diff)) {
            $this->printer->print(
                $this->prefixLines($this->prefixFor("diff", $status), $diff),
            );
        ...
        if ($throwable->hasPrevious()) {
            ...
            $this->printer->print(
                $this->prefixLines($this->prefixFor("default", $status), " "),
            );
     ...
     */
    private function colorizeMessageAndDiff(
        string $buffer,
        string $style,
    ): array {
        ...
        foreach ($lines as $line) {
            ...
            if (!$insideDiff) {
                ...
                if (str_starts_with($line, "-")) {
                    $line = Color::colorize(
                        "fg-red",
                        Color::visualizeWhitespace($line, true),
                    );
    ...
    private function formatStackTrace(string $stackTrace): string {
        ...
        foreach (explode(PHP_EOL, $stackTrace) as $line) {
            if (preg_match('/^(.*):(\d+)$/', $line, $matches)) {
                $lines[] =
                    Color::colorizePath($matches[1], $previousPath) .
                    Color::dim(":") .
     ...
     */
    private function printBeforeClassOrAfterClassErrors(
        string $type,
        array $errors,
    ): void {
        if (empty($errors)) {
        ...
        $this->printer->print(
            sprintf("These %s methods errored:" . PHP_EOL . PHP_EOL, $type),
        );
        ...
        foreach ($errors as $method => $error) {
            $this->printer->print(
                sprintf("%d) %s" . PHP_EOL, ++$index, $method),
            );

            $this->printer->print(
                trim($error->throwable()->description()) . PHP_EOL . PHP_EOL,
            );
            $this->printer->print(
                $this->formatStackTrace($error->throwable()->stackTrace()) .
                    PHP_EOL,
            );
        }
