...
final class SummaryPrinter {
    ...
    public function print(TestResult $result): void {
        ...
        if ($result->wasSuccessful()) {
            ...
            if (
                ...
                $this->printWithColor("fg-white, bg-red", "ERRORS!");
            } else {
                $this->printWithColor("fg-white, bg-red", "FAILURES!");
            }
        }

        $this->printCountString(
            $result->numberOfTestsRun(),
            "Tests",
            $color,
            true,
    ...
    private function printCountString(
        int $count,
        string $name,
        string $color,
        bool $always = false,
    ...
    private function printWithColor(
        string $color,
        string $buffer,
        bool $lf = true,
    ): void {
    ...
    private function printNumberOfIssuesIgnoredByBaseline(
        TestResult $result,
    ): void {
        if ($result->hasIssuesIgnoredByBaseline()) {
            $this->printer->print(
                sprintf(
                    ...
                    $result->numberOfIssuesIgnoredByBaseline() > 1
                        ? "were"
                        : "was",
                    PHP_EOL,
                ),
