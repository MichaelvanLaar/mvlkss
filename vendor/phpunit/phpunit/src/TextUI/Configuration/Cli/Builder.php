...
final class Builder {
        ...
        foreach ($options[0] as $option) {
            ...
            switch ($option[0]) {
                case "--colors":
                    $colors =
                        $option[1] ?:
                        \PHPUnit\TextUI\Configuration\Configuration::COLOR_AUTO;
                ...
                case "--generate-baseline":
                    ...
                    if (basename($generateBaseline) === $generateBaseline) {
                        $generateBaseline =
                            getcwd() . DIRECTORY_SEPARATOR . $generateBaseline;
                    }
                ...
                case "--use-baseline":
                    ...
                    if (
                        basename($useBaseline) === $useBaseline &&
                        !is_file($useBaseline)
                    ) {
                        $useBaseline =
                            getcwd() . DIRECTORY_SEPARATOR . $useBaseline;
                    }
                ...
                case "--covers":
                    ...
                    $testsCovering = array_merge(
                        $testsCovering,
                        array_map("strtolower", explode(",", $option[1])),
                    );
                ...
                case "--log-events-verbose-text":
                    $logEventsVerboseText = Filesystem::resolveStreamOrFile(
                        $option[1],
                    );
    ...
    private function markProcessed(string $option): void {
        ...
        if ($this->processed[$option] === 2) {
            EventFacade::emitter()->testRunnerTriggeredPhpunitWarning(
                sprintf("Option %s cannot be used more than once", $option),
            );
        }
    }
     ...
     * @param non-empty-string $option
     */
    private function warnWhenOptionsConflict(
        ?bool $current,
        string $option,
        string $opposite,
    ): void {
