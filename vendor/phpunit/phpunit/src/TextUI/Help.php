...
final class Help {
    ...
    public function __construct(?int $width = null, ?bool $withColor = null) {
        ...
        foreach ($this->elements() as $options) {
            foreach ($options as $option) {
                if (isset($option["arg"])) {
                    $this->lengthOfLongestOptionName = max(
                        $this->lengthOfLongestOptionName,
                        strlen($option["arg"]),
                    );
                }
            }
        ...
        $this->columnsAvailableForDescription =
            $width - $this->lengthOfLongestOptionName - 4;
    }
            ...
            foreach ($options as $option) {
                ...
                if (isset($option["arg"])) {
                    ...
                    $arg = preg_replace_callback(
                        "/(<[^>]+>)/",
                        static fn($matches) => Color::colorize(
                            "fg-cyan",
                            $matches[0],
                        ),
                        $arg,
                    ...
                    $buffer .=
                        self::LEFT_MARGIN . $arg . " " . $desc[0] . PHP_EOL;

                    for ($i = 1; $i < count($desc); $i++) {
                        $buffer .=
                            str_repeat(
                                " ",
                                $this->lengthOfLongestOptionName + 3,
                            ) .
                            $desc[$i] .
                            PHP_EOL;
                    }
    ...
    private function elements(): array {
        $elements = [
            "Usage" => [["text" => "phpunit [options] <directory|file> ..."]],
            ...
            "Configuration" => [
                ...
                [
                    "arg" => "--no-extensions",
                    "desc" => "Do not register test runner extensions",
                ],
                [
            ...
            "Selection" => [
                ...
                [
                    "arg" => "--requires-php-extension <name>",
                    "desc" =>
                        "Only run tests that require PHP extension <name>",
                ],
                [
                    "arg" => "--list-test-files",
                    "desc" => "List available test files",
                ],
                ...
                [
                    "arg" => "--exclude-filter <pattern>",
                    "desc" => "Exclude tests for the specified filter pattern",
                ],
                [
            ...
            "Execution" => [
                ...
                [
                    "arg" => "--do-not-report-useless-tests",
                ...
                [
                    "arg" => "--fail-on-phpunit-warning",
                    "desc" =>
                        "Signal failure using shell exit code when a PHPUnit warning was triggered",
                ],
                [
                    "arg" => "--fail-on-notice",
                    "desc" =>
                ...
                [
                    "arg" => "--do-not-fail-on-empty-test-suite",
                    "desc" =>
                        "Do not signal failure using shell exit code when no tests were run",
                ],
                [
                    "arg" => "--do-not-fail-on-warning",
                    "desc" =>
                        "Do not signal failure using shell exit code when a warning was triggered",
                ],
                [
                    "arg" => "--do-not-fail-on-risky",
                    "desc" =>
                        "Do not signal failure using shell exit code when a test was considered risky",
                ],
                [
                    "arg" => "--do-not-fail-on-deprecation",
                    "desc" =>
                        "Do not signal failure using shell exit code when a deprecation was triggered",
                ],
                [
                    "arg" => "--do-not-fail-on-phpunit-deprecation",
                    "desc" =>
                        "Do not signal failure using shell exit code when a PHPUnit deprecation was triggered",
                ],
                [
                    "arg" => "--do-not-fail-on-phpunit-warning",
                    "desc" =>
                        "Do not signal failure using shell exit code when a PHPUnit warning was triggered",
                ],
                [
                    "arg" => "--do-not-fail-on-notice",
                    "desc" =>
                        "Do not signal failure using shell exit code when a notice was triggered",
                ],
                [
                    "arg" => "--do-not-fail-on-skipped",
                    "desc" =>
                        "Do not signal failure using shell exit code when a test was skipped",
                ],
                [
                    "arg" => "--do-not-fail-on-incomplete",
                    "desc" =>
                        "Do not signal failure using shell exit code when a test was marked incomplete",
                ],
                ["spacer" => ""],
            ...
            "Code Coverage" => [
                ...
                [
                    ...
                    "desc" =>
                        "Option for code coverage report in text format: show uncovered files",
                ],
                [
        ...
        if (defined("__PHPUNIT_PHAR__")) {
            $elements["PHAR"] = [
                [
                    "arg" => "--manifest",
                    "desc" =>
                        "Print Software Bill of Materials (SBOM) in plain-text format",
                ],
                [
                    "arg" => "--sbom",
                    "desc" =>
                        "Print Software Bill of Materials (SBOM) in CycloneDX XML format",
                ],
                [
                    "arg" => "--composer-lock",
        ...
        $elements["Miscellaneous"] = [
            ...
            [
                "arg" => "--check-php-configuration",
                "desc" =>
                    "Checks whether PHP configuration follows best practices",
            ],
        ];
