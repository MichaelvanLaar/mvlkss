...
final class ReturnValueGenerator {
    ...
    public function generate(
        ...
        if (str_contains($returnType, "|")) {
            ...
            foreach (array_keys($types) as $key) {
                if (
                    str_starts_with($types[$key], "(") &&
                    str_ends_with($types[$key], ")")
                ) {
            ...
            foreach ($types as $type) {
                if (str_contains($type, "&")) {
                    ...
                    if ($this->onlyInterfaces($_types)) {
                        return $this->testDoubleForIntersectionOfInterfaces(
                            $_types,
                            $className,
                            $methodName,
                        );
     ...
     */
    private function newInstanceOf(
        StubInternal $testStub,
        string $className,
        string $methodName,
    ): Stub {
     ...
     * @throws RuntimeException
     */
    private function testDoubleFor(
        string $type,
        string $className,
        string $methodName,
    ): Stub {
     ...
     * @throws RuntimeException
     */
    private function testDoubleForIntersectionOfInterfaces(
        array $types,
        string $className,
        string $methodName,
    ): Stub {
        try {
            return (new Generator())->testDoubleForInterfaceIntersection(
                $types,
                false,
            );
            // @codeCoverageIgnoreStart
