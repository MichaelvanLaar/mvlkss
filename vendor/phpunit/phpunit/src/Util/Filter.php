...
final readonly class Filter {
     ...
     * @throws Exception
     */
    public static function stackTraceFromThrowableAsString(
        Throwable $t,
        bool $unwrap = true,
    ): string {
        ...
        if (!self::frameExists($stackTrace, $file, $line)) {
            array_unshift($stackTrace, ["file" => $file, "line" => $line]);
        }
     ...
     */
    private static function shouldPrintFrame(
        array $frame,
        false|string $prefix,
        ...
        $file = $frame["file"];
        $fileIsNotPrefixed =
            $prefix === false || !str_starts_with($file, $prefix);
    ...
    private static function fileIsExcluded(
        string $file,
        ExcludeList $excludeList,
    ): bool {
     ...
     */
    private static function frameExists(
        array $trace,
        string $file,
        int $line,
    ): bool {
