...
class Intervals {
    ...
    public static function isSubsetOf(
        ...
        if (
            $candidate instanceof MatchNoneConstraint ||
            $constraint instanceof MatchNoneConstraint
        ) {
            return false;
        }

        $intersectionIntervals = self::get(
            new MultiConstraint([$candidate, $constraint], true),
        );
        ...
        foreach ($intersectionIntervals["numeric"] as $index => $interval) {
            ...
            if (
                (string) $candidateIntervals["numeric"][$index]->getStart() !==
            ...
            if (
                (string) $candidateIntervals["numeric"][$index]->getEnd() !==
        ...
        if (
            ...
            $candidateIntervals["branches"]["exclude"]
        ) {
            return false;
        }
        if (
            \count($intersectionIntervals["branches"]["names"]) !==
            \count($candidateIntervals["branches"]["names"])
        ) {
            return false;
        }
        foreach (
            $intersectionIntervals["branches"]["names"]
            as $index => $name
        ) {
     ...
     * @return bool
     */
    public static function haveIntersections(
        ConstraintInterface $a,
        ConstraintInterface $b,
    ) {
        if (
            $a instanceof MatchAllConstraint ||
        ...
        if (
            $a instanceof MatchNoneConstraint ||
            $b instanceof MatchNoneConstraint
        ) {
            return false;
        }

        $intersectionIntervals = self::generateIntervals(
            new MultiConstraint([$a, $b], true),
            true,
        );

        return \count($intersectionIntervals["numeric"]) > 0 ||
            $intersectionIntervals["branches"]["exclude"] ||
            \count($intersectionIntervals["branches"]["names"]) > 0;
    }
    ...
    public static function compactConstraint(ConstraintInterface $constraint) {
        ...
        if (
            \count($intervals["numeric"]) === 1 &&
            (string) $intervals["numeric"][0]->getStart() ===
                (string) Interval::fromZero() &&
            (string) $intervals["numeric"][0]->getEnd() ===
                (string) Interval::untilPositiveInfinity()
        ) {
            ...
            $unEqualConstraints = [];
            for (
                $i = 0, $count = \count($intervals["numeric"]);
                $i < $count;
                $i++
            ) {
                ...
                // they are zero/+inf
                if (
                    $interval->getEnd()->getOperator() === "<" &&
                    $i + 1 < $count
                ) {
                    $nextInterval = $intervals["numeric"][$i + 1];
                    if (
                        $interval->getEnd()->getVersion() ===
                            $nextInterval->getStart()->getVersion() &&
                        $nextInterval->getStart()->getOperator() === ">"
                    ) {
                        ...
                        if (
                            \count($unEqualConstraints) === 0 &&
                            (string) $interval->getStart() !==
                                (string) Interval::fromZero()
                        ) {
                            $unEqualConstraints[] = $interval->getStart();
                        }
                        $unEqualConstraints[] = new Constraint(
                            "!=",
                            $interval->getEnd()->getVersion(),
                        );
                        continue;
                ...
                if (\count($unEqualConstraints) > 0) {
                    ...
                    if (\count($unEqualConstraints) > 1) {
                        $constraints[] = new MultiConstraint(
                            $unEqualConstraints,
                            true,
                        );
                    } else {
                ...
                if (
                    $interval->getStart()->getVersion() ===
                        $interval->getEnd()->getVersion() &&
                    $interval->getStart()->getOperator() === ">=" &&
                    $interval->getEnd()->getOperator() === "<="
                ) {
        ...
        if (0 === \count($intervals["branches"]["names"])) {
            ...
            if ($intervals["branches"]["exclude"]) {
                ...
                if (
                    \count($constraints) === 1 &&
                    (string) $constraints[0] === (string) Interval::fromZero()
                ) {
                ...
                return new MultiConstraint(
                    array_merge($constraints, $devConstraints),
                    true,
                );
            }
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}
     */
    private static function generateIntervals(
        ConstraintInterface $constraint,
        ...
        if ($constraint instanceof MatchAllConstraint) {
            return [
                "numeric" => [
                    new Interval(
                        Interval::fromZero(),
                        Interval::untilPositiveInfinity(),
                    ),
                ],
        ...
        if (!($constraint instanceof MultiConstraint)) {
            throw new \UnexpectedValueException(
                "The constraint passed in should be an MatchAllConstraint, Constraint or MultiConstraint instance, got " .
            ...
            foreach ($constraintBranches as $b) {
                if ($b["exclude"]) {
                    if ($branches["exclude"]) {
                        ...
                        // !=a,!=b || !=b,!=c => !=b
                        $branches["names"] = array_intersect(
                            $branches["names"],
                            $b["names"],
                        );
                    } else {
        ...
        foreach ($numericGroups as $group) {
            foreach ($group as $interval) {
                $borders[] = [
                    "version" => $interval->getStart()->getVersion(),
                    "operator" => $interval->getStart()->getOperator(),
                    "side" => "start",
                ];
                $borders[] = [
                    "version" => $interval->getEnd()->getVersion(),
                    "operator" => $interval->getEnd()->getOperator(),
                    "side" => "end",
                ];
            }
        }
        ...
        usort($borders, function ($a, $b) use ($opSortOrder) {
            ...
            if ($order === 0) {
                return $opSortOrder[$a["operator"]] -
                    $opSortOrder[$b["operator"]];
            }
        ...
        $intervals = [];
        $index = 0;
        $activationThreshold = $constraint->isConjunctive()
            ? \count($numericGroups)
            : 1;
        $start = null;
            ...
            if (!$start && $activeIntervals >= $activationThreshold) {
                ...
                if (
                    ...
                    $intervals[$index] = new Interval(
                        $start,
                        new Constraint($border["operator"], $border["version"]),
                    );
                    $index++;
     ...
     * @phpstan-return array{'numeric': Interval[], 'branches': array{'names': string[], 'exclude': bool}}
     */
    private static function generateSingleConstraintIntervals(
        ...
        if ($op[0] === "<") {
            // < & <=
            return [
                "numeric" => [new Interval(Interval::fromZero(), $constraint)],
                "branches" => Interval::noDev(),
            ];
        }
        if ($op === "!=") {
            ...
            return [
                "numeric" => [
                    new Interval(
                        Interval::fromZero(),
                        new Constraint("<", $constraint->getVersion()),
                    ),
                    new Interval(
                        new Constraint(">", $constraint->getVersion()),
                        Interval::untilPositiveInfinity(),
