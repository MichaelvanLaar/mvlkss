...
class SimpleImage {
    ...
    public function fromDataUri(string $uri): static {
        ...
        // Get image data
        $data = base64_decode(
            strval(preg_replace("/^data:(.*?);base64,/", "", $uri)),
        );
    ...
    public function generate(
        ...
        switch ($mimeType) {
            ...
            case "image/png":
                ...
                if ($compression !== -1) {
                    $compression = (int) round(
                        self::keepWithin($compression, 0, 10),
                    );
                }
                imagesavealpha($this->image, $alpha);
    ...
    public function toFile(
        ...
        if (!file_put_contents($file, $image["data"])) {
            throw new Exception(
                "Failed to write image to file: $file",
                self::ERR_WRITE,
            );
        }
     ...
     */
    protected static function keepWithin(
        int|float $value,
        int|float $min,
        int|float $max,
    ): int|float {
     ...
     */
    protected static function imageCopyMergeAlpha(
        GdImage $dstIm,
        GdImage $srcIm,
        int $dstX,
        int $dstY,
    ...
    public function overlay(
        ...
        // Set default center
        $x = (int) round(
            $spaceX / 2 + ($calculateOffsetFromEdge ? 0 : $xOffset),
        );
        $y = (int) round(
            $spaceY / 2 + ($calculateOffsetFromEdge ? 0 : $yOffset),
        );
    ...
    public function text(
        ...
        if ($baselineAlign) {
            ...
            if (str_contains($anchor, "bottom")) {
                ...
                $yOffset +=
                    $boxFullHeight / 2 - $boxHeight / 2 - abs($boxFull[1]);
            }
        } else {
    ...
    public function textBox(string $text, array $options): static {
        ...
        [$lines, $isLastLine, $lastLineHeight] = self::textSeparateLines(
            $text,
            $fontFile,
            $fontSize,
            $maxWidth,
        ...
        $maxHeight = (int) round(
            ((is_countable($lines) ? count($lines) : 0) - 1) *
                ($fontSizePx * 1.2 + $leading) +
                $lastLineHeight,
        );
        ...
        if ($align != "justify") {
            ...
            foreach ($lines as $keyLine => $line) {
                ...
                if ($countWords > 1) {
                    $wordSpacing =
                        ($maxWidth - $wordsSizeTotal) / ($countWords - 1);
     ...
     */
    private function textSeparateLines(
        string $text,
        string $fontFile,
        float $fontSize,
        int $maxWidth,
    ...
    public function arc(
        ...
        if ($thickness === "filled") {
            ...
            $tempColor =
                self::normalizeColor($color)["red"] == 255 ? "blue" : "red";
    ...
    public function roundedRectangle(
        ...
        if ($thickness == "filled") {
            ...
            $y1 -= $offset;
            $y2 += $offset;
            $radius = self::keepWithin(
                $radius,
                0,
                min(($x2 - $x1) / 2, ($y2 - $y1) / 2) - 1,
            );
            $radius = (int) floor($radius);
            $thickness = self::keepWithin(
                $thickness,
                1,
                min(($x2 - $x1) / 2, ($y2 - $y1) / 2),
            );
     ...
     * @throws Exception
     */
    private function excludeInsideColor(
        int $x,
        int $y,
    ...
    public function contrast(int $percentage): static {
        imagefilter(
            $this->image,
            IMG_FILTER_CONTRAST,
            self::keepWithin($percentage, -100, 100),
        );
     ...
     * @throws Exception
     */
    public static function adjustColor(
        string|array $color,
        int $red,
        int $green,
        int $blue,
     ...
     * @throws Exception
     */
    public static function darkenColor(
        string|array $color,
        int $amount,
    ): array {
     ...
     * @throws Exception
     */
    public static function lightenColor(
        string|array $color,
        int $amount,
    ): array {
        return self::adjustColor($color, $amount, $amount, $amount, 0);
    ...
    public static function normalizeColor(string|array $color): array {
        ...
        if (
            is_string($color) &&
            array_key_exists(strtolower($color), $cssColors)
        ) {
