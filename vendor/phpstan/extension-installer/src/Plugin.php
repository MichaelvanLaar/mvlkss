...
final class Plugin implements PluginInterface, EventSubscriberInterface {
    ...
    public function process(Event $event): void {
        ...
        if (!file_exists(__DIR__)) {
            $io->write(
                "<info>phpstan/extension-installer:</info> Package not found (probably scheduled for removal); extensions installation skipped.",
            );
            return;
        }
        ...
        foreach (
            ...
            if (array_key_exists("phpstan/phpstan", $packageRequires)) {
                $phpstanConstraint = $packageRequires[
                    "phpstan/phpstan"
                ]->getConstraint();
            ...
            $data[$package->getName()] = [
                ...
                "relative_install_path" => $fs->findShortestPath(
                    dirname($generatedConfigFilePath),
                    $absoluteInstallPath,
                    true,
                ),
                "extra" => $package->getExtra()["phpstan"] ?? null,
                "version" => $package->getFullPrettyVersion(),
                "phpstanVersionConstraint" =>
                    $phpstanConstraint !== null
                        ? $this->constraintIntoString($phpstanConstraint)
                        : null,
            ];
        ...
        if (
            count($phpstanVersionConstraints) > 0 &&
            class_exists(Intervals::class)
        ) {
            if (count($phpstanVersionConstraints) === 1) {
                $multiConstraint = $phpstanVersionConstraints[0];
            } else {
                $multiConstraint = new MultiConstraint(
                    $phpstanVersionConstraints,
                );
            }
            $phpstanVersionConstraint = $this->constraintIntoString(
                Intervals::compactConstraint($multiConstraint),
            );
        }
        ...
        $generatedConfigFileContents = sprintf(
            ...
            var_export($phpstanVersionConstraint, true),
        );
        file_put_contents(
            $generatedConfigFilePath,
            $generatedConfigFileContents,
        );
        $io->write(
            "<info>phpstan/extension-installer:</info> Extensions installed",
        );
        ...
        foreach (array_keys($notInstalledPackages) as $name) {
            $io->write(
                sprintf("> <comment>%s:</comment> not supported", $name),
            );
        }
