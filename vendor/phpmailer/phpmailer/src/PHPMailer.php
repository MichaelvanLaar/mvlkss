...
class PHPMailer {
    ...
    public static function parseAddresses(
        ...
        if ($useimap !== null) {
            trigger_error(
                self::lang("deprecated_argument") . '$useimap',
                E_USER_DEPRECATED,
            );
        }
        $addresses = [];
            ...
            foreach ($list as $address) {
                if (
                    ...
                    if (
                        ...
                        $address->personal = static::decodeHeader(
                            $address->personal,
                            $charset,
                        );
                    }
        ...
        try {
            ...
            if (
                ...
                ($this->anyAddressHasUnicodeLocalPart($this->RecipientsQueue) ||
                    $this->anyAddressHasUnicodeLocalPart(
    ...
    protected function sendmailSend($header, $body) {
        ...
        $resultArgs = empty($sendmailArgs)
            ? ""
            : " " . implode(" ", $sendmailArgs);
        ...
        if ($this->SingleTo) {
            foreach ($this->SingleToArray as $toAddr) {
                ...
                if (!$mail) {
                    throw new Exception(
                        self::lang("execute") . $this->Sendmail,
                        self::STOP_CRITICAL,
                    );
                }
    ...
    public function smtpConnect($options = null) {
        ...
        foreach ($hosts as $hostentry) {
            ...
            if (
                ...
                $this->edebug(
                    self::lang("invalid_hostentry") . " " . trim($hostentry),
                );
            ...
            if (
                ...
                if (!$sslext) {
                    throw new Exception(
                        self::lang("extension_missing") . "openssl",
                        self::STOP_CRITICAL,
                    );
                }
            }
        ...
        if ($this->isError()) {
            ...
            try {
                ...
                if ($sign) {
                    ...
                    @unlink($signed);
                    throw new Exception(
                        self::lang("signing") . openssl_error_string(),
                    );
                }
            } catch (Exception $exc) {
                $body = "";
        ...
        if ("smtp" === $this->Mailer && null !== $this->smtp) {
            ...
            if (!empty($lasterror["error"])) {
                ...
                if (!empty($lasterror["detail"])) {
                    $msg .=
                        " " . self::lang("smtp_detail") . $lasterror["detail"];
                }
                if (!empty($lasterror["smtp_code"])) {
                    $msg .=
                        " " . self::lang("smtp_code") . $lasterror["smtp_code"];
                }
                if (!empty($lasterror["smtp_code_ex"])) {
                    $msg .=
                        " " .
                        self::lang("smtp_code_ex") .
        ...
        if (array_key_exists(2, $images)) {
            ...
            foreach ($images[2] as $imgindex => $url) {
                ...
                if (
                    ...
                    $cid =
                        substr(hash("sha256", $data), 0, 32) .
                        "@" .
                        $cid_domain; //RFC2392 S 2
                ...
                if (
                    ...
                    //RFC2392 S 2
                    $cid =
                        substr(hash("sha256", $url), 0, 32) . "@" . $cid_domain;
                    if (strlen($basedir) > 1 && "/" !== substr($basedir, -1)) {
                        $basedir .= "/";
                    }
                    if (
