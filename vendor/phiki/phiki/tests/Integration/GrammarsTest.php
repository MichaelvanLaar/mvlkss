...
describe("Grammars", function () {
    test("default grammars do not produce warnings or exceptions", function (
        string $grammar,
    ) {
        $sample = file_get_contents(
            __DIR__ . "/../../resources/samples/" . $grammar . ".sample",
        );
...
dataset("grammars", function () {
    ...
    $grammars = array_filter(
        $repository->getAllGrammarNames(),
        fn(string $grammar) => !in_array($grammar, [
            "astro",
            "haxe",
            "fluent",
            "stylus",
