...
test("a single scope path correctly matches a single scope", function () {
    $path = new Path([new Scope(["text", "html", "blade", "php"])]);
...
test("a group correctly matches a set of scopes", function () {
    $group = new Group(
        new Selector([
            new Composite([
                new Expression(
                    new Path([new Scope(["text", "html", "blade", "php"])]),
                ),
            ]),
        ]),
    );
...
test("a scope with wildcards can match another scope", function () {
    ...
    expect(
        $scope->matches(Scope::fromString("text.html.blade.php")),
    )->toBeTrue();
    expect(
        $scope->matches(Scope::fromString("text.html.twig.php")),
    )->toBeTrue();
});
