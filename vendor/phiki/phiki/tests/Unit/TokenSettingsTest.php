...
describe("TokenSettings", function () {
    it("can be constructed", function () {
        expect(new TokenSettings(null, null, null))->toBeInstanceOf(
            TokenSettings::class,
        );
    });
    ...
    it(
        "can generate a style string with font style strikethrough",
        function () {
            $settings = new TokenSettings(null, null, "strikethrough");

            expect($settings->toStyleString())->toBe(
    ...
    it(
        "can generate a style string with multiple font styles in any order",
        function () {
    ...
    it(
        "can generate a style string with multiple font styles and foreground",
        function () {
    ...
    it(
        "can generate a style string with multiple font styles and background",
        function () {
            $settings = new TokenSettings("#000", null, "underline italic");

            expect($settings->toStyleString())->toBe(
                "background-color: #000;text-decoration: underline;font-style: italic;",
            );
        },
    );
});
