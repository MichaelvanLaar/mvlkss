...
function main() {
    ...
    eachFile($grammarsDirectory, function (SplFileInfo $grammar) use (
        &$grammars,
    ) {
        $json = file_get_contents($grammar->getRealPath());
        $basename = rtrim(
            basename($grammar->getFilename(), $grammar->getExtension()),
            ".",
        );
        ...
        file_put_contents(
            __DIR__ . "/../resources/languages/" . $grammar->getFilename(),
            $json,
        );
    ...
    eachFile($themesDirectory, function (SplFileInfo $theme) use (&$themes) {
        copy(
            $theme->getRealPath(),
            __DIR__ . "/../resources/themes/" . $theme->getFilename(),
        );
        ...
        $themes[] = [
            ...
            "name" =>
                $json["name"] ??
                basename($theme->getFilename(), $theme->getExtension()),
        ];
    });

    echo "Generating DefaultGrammars class...\n";

    $defaultGrammarsStub = file_get_contents(
        __DIR__ . "/stubs/DefaultGrammars.php.stub",
    );
    $namesToPaths = [];
    $scopesToNames = [];

    foreach ($grammars as $grammar) {
        $namesToPaths[] = sprintf(
            '"%s" => __DIR__ . "/../../resources/languages/%s"',
            $grammar["name"],
            $grammar["path"],
        );
        $scopesToNames[$grammar["scopeName"]] = sprintf(
            '"%s" => "%s"',
            $grammar["scopeName"],
            $grammar["name"],
        );
    ...
    file_put_contents(
        __DIR__ . "/../src/Grammar/DefaultGrammars.php",
        $defaultGrammarsStub,
    );
    ...
    foreach ($grammars as $grammar) {
        $grammarCases[] = sprintf(
            'case %s = "%s";',
            Str::studly($grammar["name"]),
            $grammar["name"],
        );
    }
    ...
    file_put_contents(
        __DIR__ . "/../src/Grammar/Grammar.php",
        $grammarEnumStub,
    );

    echo "Generating DefaultThemes class...\n";

    $defaultThemesStub = file_get_contents(
        __DIR__ . "/stubs/DefaultThemes.php.stub",
    );
    $namesToPaths = [];

    foreach ($themes as $theme) {
        $namesToPaths[] = sprintf(
            '"%s" => __DIR__ . "/../../resources/themes/%s"',
            $theme["name"],
            $theme["path"],
        );
    }
    ...
    file_put_contents(
        __DIR__ . "/../src/Theme/DefaultThemes.php",
        $defaultThemesStub,
    );
