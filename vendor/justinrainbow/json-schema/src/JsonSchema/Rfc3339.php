...
class Rfc3339 {
    ...
    public static function createFromString($input): ?\DateTime {
        ...
        $dateFormat =
            $inputHasTSeparator === false
                ? "Y-m-d H:i:s.uP"
                : "Y-m-d\TH:i:s.uP";
        $dateTime = \DateTimeImmutable::createFromFormat(
            $dateFormat,
            $dateAndTime . $microseconds . $timeZone,
        ...
        if (
            $matches[4] === "60" &&
            $utcDateTime->sub($oneSecond)->format("H:i:s") === "23:59:59"
        ) {
        ...
        if (
            $dateTime->format(
                $inputHasTSeparator ? "Y-m-d\TH:i:s" : "Y-m-d H:i:s",
            ) !== $matches[1]
        ) {
            return null;
        }
        ...
        if ($mutable === false) {
            throw new \RuntimeException(
                "Unable to create DateTime from DateTimeImmutable",
            );
        }
