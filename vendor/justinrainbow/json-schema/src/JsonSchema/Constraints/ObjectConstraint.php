...
class ObjectConstraint extends Constraint {
    ...
    public function check(
        ...
        if ($patternProperties) {
            ...
            $matches = $this->validatePatternProperties(
                $element,
                $path,
                $patternProperties,
            );
    ...
    public function validatePatternProperties(
        $element,
        ?JsonPointer $path,
        $patternProperties,
    ) {
        ...
        foreach ($patternProperties as $pregex => $schema) {
            ...
            if (@preg_match($fullRegex, "") === false) {
                $this->addError(ConstraintError::PREGEX_INVALID(), $path, [
                    "pregex" => $pregex,
                ]);
                continue;
            }
            foreach ($element as $i => $value) {
                if (preg_match($fullRegex, (string) $i)) {
                    ...
                    $this->checkUndefined(
                        ...
                        $path,
                        $i,
                        in_array($i, $this->appliedDefaults),
                    );
                }
            }
        }
    ...
    public function validateElement(
        ...
        foreach ($element as $i => $value) {
            ...
            if (
                ...
                $this->inlineSchemaProperty !== $i &&
                !$definition
            ) {
                $this->addError(
                    ConstraintError::ADDITIONAL_PROPERTIES(),
                    $path,
                    ["property" => $i],
                );
            }
            ...
            $property = $this->getProperty(
                $element,
                $i,
                $this->factory->createInstanceFor("undefined"),
            );
            if (is_object($property)) {
                $this->validateMinMaxConstraint(
                    !($property instanceof UndefinedConstraint)
                        ? $property
                        : $element,
                    $definition,
                    $path,
        ...
        if (
            ...
            (isset($element[$property]) ||
                array_key_exists(
                    $property,
                    $element,
                )) /*$this->checkMode == self::CHECK_MODE_TYPE_CAST*/
        ) {
            return $element[$property];
        } elseif (
            is_object($element) &&
            property_exists($element, (string) $property)
        ) {
            return $element->$property;
        }
     ...
     */
    protected function validateMinMaxConstraint(
        $element,
        $objectDefinition,
        ?JsonPointer $path = null,
    ) {
        ...
        if (
            isset($objectDefinition->minProperties) &&
            is_int($objectDefinition->minProperties)
        ) {
            if (
                $this->getTypeCheck()->propertyCount($element) <
                max(0, $objectDefinition->minProperties)
            ) {
                $this->addError(ConstraintError::PROPERTIES_MIN(), $path, [
        ...
        if (
            isset($objectDefinition->maxProperties) &&
            is_int($objectDefinition->maxProperties)
        ) {
            if (
                $this->getTypeCheck()->propertyCount($element) >
                max(0, $objectDefinition->maxProperties)
            ) {
                $this->addError(ConstraintError::PROPERTIES_MAX(), $path, [
