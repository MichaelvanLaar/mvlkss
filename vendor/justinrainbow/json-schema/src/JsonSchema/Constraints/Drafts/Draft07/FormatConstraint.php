...
class FormatConstraint implements ConstraintInterface {
        ...
        switch ($schema->format) {
            ...
            case "date-time":
                if (!$this->validateRfc3339DateTime($value)) {
                    $this->addError(
                        ...
                        $path,
                        ["dateTime" => $value, "format" => $schema->format],
                    );
                }
                break;
            case "utc-millisec":
            ...
            case "ipv4":
                if (
                    filter_var(
                        $value,
                        FILTER_VALIDATE_IP,
                        FILTER_NULL_ON_FAILURE | FILTER_FLAG_IPV4,
                    ) === null
                ) {
                    $this->addError(ConstraintError::FORMAT_IP(), $path, [
                        "format" => $schema->format,
                    ]);
                }
                break;
            case "ipv6":
                if (
                    filter_var(
                        $value,
                        FILTER_VALIDATE_IP,
                        FILTER_NULL_ON_FAILURE | FILTER_FLAG_IPV6,
                    ) === null
                ) {
                    $this->addError(ConstraintError::FORMAT_IP(), $path, [
                        "format" => $schema->format,
            ...
            case "color":
                if (!$this->validateColor($value)) {
                    $this->addError(ConstraintError::FORMAT_COLOR(), $path, [
                        "format" => $schema->format,
                    ]);
                }
                break;
            case "style":
                if (!$this->validateStyle($value)) {
                    $this->addError(ConstraintError::FORMAT_STYLE(), $path, [
                        "format" => $schema->format,
                    ]);
                }
                break;
            case "phone":
                if (!$this->validatePhone($value)) {
                    $this->addError(ConstraintError::FORMAT_PHONE(), $path, [
                        "format" => $schema->format,
                    ]);
                }
                break;
            ...
            case "uri-reference":
                if (
                    !(
                        UriValidator::isValid($value) ||
                        RelativeReferenceValidator::isValid($value)
                    )
                ) {
                    $this->addError(ConstraintError::FORMAT_URL(), $path, [
                        "format" => $schema->format,
            ...
            case "uri-template":
                if (!$this->validateUriTemplate($value)) {
                    $this->addError(
                        ConstraintError::FORMAT_URI_TEMPLATE(),
                        $path,
                        ["format" => $schema->format],
                    );
                }
            ...
            case "email":
                if (
                    filter_var(
                        ...
                        FILTER_NULL_ON_FAILURE | FILTER_FLAG_EMAIL_UNICODE,
                    ) === null
                ) {
                    $this->addError(ConstraintError::FORMAT_EMAIL(), $path, [
                        "format" => $schema->format,
                    ]);
                }
                break;
            ...
            case "hostname":
                if (!$this->validateHostname($value)) {
                    $this->addError(ConstraintError::FORMAT_HOSTNAME(), $path, [
                        "format" => $schema->format,
                    ]);
                }
                break;
            ...
            case "json-pointer":
                if (!$this->validateJsonPointer($value)) {
                    $this->addError(
                        ConstraintError::FORMAT_JSON_POINTER(),
                        $path,
                        ["format" => $schema->format],
                    );
                }
    ...
    private function validateInternationalizedHostname(string $host): bool {
        ...
        foreach ($labels as $label) {
            ...
            if (
                preg_match("/[\x{05F3}\x{05F4}]/u", $label) &&
            ...
            if (
                preg_match("/[\x{0964}\x{0965}]/u", $label) &&
