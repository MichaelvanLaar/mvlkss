...
abstract class Enum implements \Stringable {
    ...
    final public function __sleep() {
        throw new LogicException(
            "Serialization is not supported by default in this pseudo-enum implementation",
        );
    }
    ...
    final public function getOrdinal() {
        if ($this->ordinal === null) {
            ...
            $value = $this->value;
            $constants =
                self::$constants[static::class] ?? static::getConstants();
    ...
    final public static function byValue($value) {
        ...
        /** @var static $instance */
        $instance =
            self::$instances[static::class][$name] ??
            (self::$instances[static::class][$name] = new static(
                $constants[$name],
            ));
        ...
        do {
            ...
            foreach ($reflection->getReflectionConstants() as $reflConstant) {
                if ($reflConstant->isPublic()) {
                    $scopeConstants[
                        $reflConstant->getName()
                    ] = $reflConstant->getValue();
                }
            }
