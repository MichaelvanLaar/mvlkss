...
abstract class PrettyPrinterAbstract implements PrettyPrinter {
    ...
    public function __construct(array $options = []) {
        $this->phpVersion =
            $options["phpVersion"] ?? PhpVersion::fromComponents(7, 4);
        ...
        $this->shortArraySyntax =
            $options["shortArraySyntax"] ??
            $this->phpVersion->supportsShortArraySyntax();
        $this->docStringEndToken = $this->phpVersion->supportsFlexibleHeredoc()
            ? null
            : "_DOC_STRING_END_" . mt_rand();
        ...
        if ($indent === "\t") {
            ...
            throw new \LogicException(
                'Option "indent" must either be all spaces or a single tab',
            );
        }
    }
    ...
    protected function pInfixOp(
        ...
        return $prefix .
            $this->p($leftNode, $newPrecedenceLHS, $newPrecedenceLHS) .
            $operatorString .
            $this->p($rightNode, $newPrecedenceRHS, $lhsPrecedence) .
            $suffix;
    }
    ...
    protected function pPostfixOp(
        ...
        return $prefix .
            $this->p($node, $opPrecedence, $lhsPrecedence) .
            $operatorString .
            $suffix;
    }
     ...
     */
    protected function pCommaSeparatedMultiline(
        array $nodes,
        bool $trailingComma,
    ): string {
        $this->indent();
    ...
    protected function pFallback(
        Node $node,
        int $precedence,
        int $lhsPrecedence,
    ): string {
        return $this->{"p" . $node->getType()}(
            $node,
            $precedence,
            $lhsPrecedence,
        );
    ...
    protected function p(
        ...
        $indentAdjustment =
            $this->indentLevel -
            $this->origTokens->getIndentationBefore($startPos);
        ...
        foreach ($node->getSubNodeNames() as $subNodeName) {
            ...
            if (
                ...
                [$printFn, $skipToken, $findToken] = $this->modifierChangeMap[
                    $key
                ];
            ...
            if ($origSubNode !== null) {
                ...
                [
                    ...
                    $extraLeft,
                    $extraRight,
                ] = $this->insertionMap[$key];
                if (null !== $findToken) {
                    $subStartPos =
                        $this->origTokens->findRight($pos, $findToken) +
                        (int) !$beforeToken;
                } else {
                    $subStartPos = $pos;
                }
                ...
                if (null === $extraLeft && null !== $extraRight) {
                    ...
                    $subStartPos = $this->origTokens->skipRightWhitespace(
                        $subStartPos,
                    );
                }
            ...
            if (null !== $subNode) {
                ...
                $this->setIndentLevel(
                    max(
                        $this->origTokens->getIndentationBefore($subStartPos) +
                            $indentAdjustment,
                        0,
                    ),
                );
    ...
    protected function pArray(
        ...
        foreach ($diff as $i => $diffElem) {
            ...
            if (
                $diffType === DiffElem::TYPE_KEEP ||
                $diffType === DiffElem::TYPE_REPLACE
            ) {
                ...
                $origIndentLevel = $this->indentLevel;
                $lastElemIndentLevel = max(
                    $this->origTokens->getIndentationBefore($itemStartPos) +
                        $indentAdjustment,
                    0,
                );
                    ...
                    foreach ($delayedAdd as $delayedAddNode) {
                        if ($insertNewline) {
                            ...
                            if ($delayedAddComments) {
                                $result .=
                                    $this->pComments($delayedAddComments) .
                                    $this->nl;
                            }
                        }
            ...
            if (
                null !== $fixup &&
                $arrItem->getAttribute("origNode") !== $origArrItem
            ) {
                $res = $this->pFixup(
                    $fixup,
                    $arrItem,
