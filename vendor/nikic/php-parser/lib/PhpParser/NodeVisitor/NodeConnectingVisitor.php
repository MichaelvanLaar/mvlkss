...
final class NodeConnectingVisitor extends NodeVisitorAbstract {
    ...
    public function enterNode(Node $node) {
        ...
        if ($this->previous !== null) {
            if ($this->weakReferences) {
                if (
                    $this->previous->getAttribute("weak_parent") ===
                    $node->getAttribute("weak_parent")
                ) {
                    $node->setAttribute(
                        "weak_previous",
                        \WeakReference::create($this->previous),
                    );
                    $this->previous->setAttribute(
                        "weak_next",
                        \WeakReference::create($node),
                    );
                }
            } elseif (
                $this->previous->getAttribute("parent") ===
