...
abstract class ParserAbstract implements Parser {
    ...
    public function parse(
        ...
        if ($result !== null) {
            $traverser = new NodeTraverser(
                new CommentAnnotatingVisitor($this->tokens),
            );
                ...
                if ($rule !== $this->unexpectedTokenRule) {
                    ...
                    try {
                        ...
                        if ($callback !== null) {
                            ...
                            $this->semValue =
                                $this->semStack[$stackPos - $ruleLength + 1];
                        }
                    } catch (Error $e) {
                        if (-1 === $e->getStartLine()) {
                            $e->setStartLine(
                                $this->tokens[$this->tokenPos]->line,
                            );
                        }
                    ...
                    $idx =
                        $this->gotoBase[$nonTerminal] + $stateStack[$stackPos];
                    if (
                        $idx >= 0 &&
                        $idx < $this->gotoTableSize &&
                    ...
                    switch ($this->errorState) {
                        ...
                        case 2:
                            ...
                            while (
                                !(
                                    (($idx =
                                        $this->actionBase[$state] +
                                        $this->errorSymbol) >= 0 &&
                                        $idx < $this->actionTableSize &&
                                        $this->actionCheck[$idx] ===
                                            $this->errorSymbol) ||
                                    ($state < $this->YY2TBLSTATE &&
                                        ($idx =
                                            $this->actionBase[
                            ...
                            $this->tokenStartStack[$stackPos] = $this->tokenPos;
                            $this->tokenEndStack[$stackPos] =
                                $this->tokenEndStack[$stackPos - 1];
                            break;
    ...
    protected function getExpectedTokens(int $state): array {
        ...
        foreach ($this->symbolToName as $symbol => $name) {
            ...
            if (
                ...
                ($state < $this->YY2TBLSTATE &&
                    ($idx =
                        $this->actionBase[$state + $this->numNonLeafStates] +
                        $symbol) >= 0 &&
                    $idx < $this->actionTableSize &&
                    $this->actionCheck[$idx] === $symbol)
            ) {
    ...
    */
    ...
    private function fixupNamespaceAttributes(
        Node\Stmt\Namespace_ $stmt,
    ): void {
        ...
        foreach ($endAttributes as $endAttribute) {
            if ($lastStmt->hasAttribute($endAttribute)) {
                $stmt->setAttribute(
                    $endAttribute,
                    $lastStmt->getAttribute($endAttribute),
                );
            }
        }
    }
    ...
    private function getNamespaceErrorAttributes(Namespace_ $node): array {
        ...
        if (isset($attrs["startFilePos"])) {
            $attrs["endFilePos"] =
                $attrs["startFilePos"] + \strlen("namespace") - 1;
        }
        return $attrs;
    }
    ...
    protected function parseLNumber(
        string $str,
        array $attributes,
        bool $allowInvalidOctal = false,
    ): Int_ {
        try {
                    ...
                    $this->emitError(
                        new Error(
                            "Invalid body indentation level " .
                                "(expecting an indentation level of at least " .
                                $indentLen .
                                ")",
                            $attributes,
                        ),
    ...
    protected function parseDocString(
        ...
        $kind =
            strpos($startToken, "'") === false
                ? String_::KIND_HEREDOC
                : String_::KIND_NOWDOC;

        $regex =
            '/\A[bB]?<<<[ \t]*[\'"]?([a-zA-Z_\x7f-\xff][a-zA-Z0-9_\x7f-\xff]*)[\'"]?(?:\r\n|\n|\r)\z/';
        ...
        if (\is_string($contents)) {
            ...
            if ($kind === String_::KIND_HEREDOC) {
                $contents = String_::parseEscapeSequences(
                    $contents,
                    null,
                    $parseUnicodeEscape,
                );
            ...
            foreach ($contents as $i => $part) {
                if ($part instanceof Node\InterpolatedStringPart) {
                    ...
                    $part->value = String_::parseEscapeSequences(
                        $part->value,
                        null,
                        $parseUnicodeEscape,
                    );
    ...
    protected function createCommentFromToken(
        Token $token,
        int $tokenPos,
    ): Comment {
    ...
    protected function maybeCreateNop(
        int $tokenStartPos,
        int $tokenEndPos,
    ): ?Nop {
    ...
    protected function fixupArrayDestructuring(Array_ $node): Expr\List_ {
        ...
        return new Expr\List_(
            array_map(function (Node\ArrayItem $item) {
                if ($item->value instanceof Expr\Error) {
    ...
    protected function checkClassMethod(
        ClassMethod $node,
        int $modifierPos,
    ): void {
    ...
    protected function checkClassConst(
        ClassConst $node,
        int $modifierPos,
    ): void {
        foreach (
            [Modifiers::STATIC, Modifiers::ABSTRACT, Modifiers::READONLY]
            as $modifier
        ) {
            if ($node->flags & $modifier) {
                $this->emitError(
                    new Error(
                        "Cannot use '" .
                            Modifiers::toString($modifier) .
                            "' as constant modifier",
                        $this->getAttributesAt($modifierPos),
                    ),
                );
            }
    ...
    protected function checkPropertyHooksForMultiProperty(
        Property $property,
        int $hookPos,
    ): void {
    ...
    protected function checkEmptyPropertyHookList(
        array $hooks,
        int $hookPos,
    ): void {
        if (empty($hooks)) {
    ...
    protected function checkPropertyHook(
        PropertyHook $hook,
        ?int $paramListPos,
    ): void {
    ...
    protected function checkPropertyHookModifiers(
        int $a,
        int $b,
        int $modifierPos,
    ): void {
        ...
        if ($b != Modifiers::FINAL) {
            $this->emitError(
                new Error(
                    "Cannot use the " .
                        Modifiers::toString($b) .
                        " modifier on a property hook",
                    $this->getAttributesAt($modifierPos),
                ),
            );
        }
     ...
     */
    protected function createExitExpr(
        string $name,
        int $namePos,
        array $args,
        array $attrs,
        ...
        if ($this->isSimpleExit($args)) {
            ...
            $attrs["kind"] =
                strtolower($name) === "exit"
                    ? Expr\Exit_::KIND_EXIT
                    : Expr\Exit_::KIND_DIE;
            return new Expr\Exit_(
                \count($args) === 1 ? $args[0]->value : null,
                $attrs,
            );
        }
        return new Expr\FuncCall(
            new Name($name, $this->getAttributesAt($namePos)),
            $args,
            $attrs,
        );
    }
    ...
    protected function createTokenMap(): array {
        ...
        foreach ($this->symbolToName as $name) {
            if ($name[0] === "T") {
                $tokenMap[\constant($name)] = constant(
                    static::class . "::" . $name,
                );
            }
        }
