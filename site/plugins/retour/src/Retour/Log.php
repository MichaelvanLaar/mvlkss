...
class Log {
    ...
    public function add(array $props): int|false {
        return $this->table()->insert([
            ...
            "path" => $props["path"],
            "referrer" =>
                $props["referrer"] ?? ($_SERVER["HTTP_REFERER"] ?? null),
    ...
    public function flush(string $mode = "all"): bool {
        $deleted = match ($mode) {
            ...
            "failures" => $this->table()->delete(
                "redirect IS NULL AND wasResolved IS NULL",
            ),
            default => false,
        };
    ...
    public function resolve(string $path): bool {
        return $this->table()->update(["wasResolved" => 1], ["path" => $path]);
    }
