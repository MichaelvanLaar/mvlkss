...
class Redirects extends Collection {
    public function __construct(protected Retour $retour, array $redirects) {
    ...
    public function toData(string $from, string $to): array {
        ...
        return $this->toArray(function (Redirect $redirect) use (
            $retour,
            $from,
            $to,
        ): array {
    ...
    public function toRoutes(bool $priority = false): array {
        ...
        $redirects = $this->filter(
            fn(Redirect $redirect): bool => $redirect->isActive() === true &&
