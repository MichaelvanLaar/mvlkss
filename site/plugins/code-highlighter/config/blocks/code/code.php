...
$lang = (string) $block
    ->language()
    ->or(option("bogdancondorachi.code-highlighter.language"));
$theme = (string) $block
    ->theme()
    ->or(option("bogdancondorachi.code-highlighter.theme"));
...
if (
    !is_null($themes) &&
    (!is_array($themes) || array_values($themes) === $themes)
) {
    error_log(
        "code-highlighter: bogdancondorachi.code-highlighter.themes must be an associative array — ignoring",
    );
    $themes = null;
}
...
$effectiveTheme =
    $themes && is_array($themes)
        ? ($block->theme()->isNotEmpty()
            ? $theme
            : $themes)
        : $theme;
...
try {
    $highlightedCode = $highlighter->codeToHtml(
        $code,
        $lang,
        $effectiveTheme,
        $gutter,
    ...
    error_log(
        'code-highlighter: unrecognised language "' .
            $lang .
            '" — ' .
            $e->getMessage(),
    );
    $highlightedCode =
        '<pre class="phiki"><code>' . htmlspecialchars($code) . "</code></pre>";
} catch (UnrecognisedThemeException $e) {
    error_log(
        'code-highlighter: unrecognised theme "' .
            $effectiveTheme .
            '" — ' .
            $e->getMessage(),
    );
